...
class _PdfPagePdfium extends PdfPage {
  ...
  static PdfLink? _createLinkForAnnot(
    ...
    if (dest != nullptr) {
      return PdfLink([rect], dest: _pdfDestFromDest(dest, document, arena), annotation: annotation);
    }

    final uri = _processAnnotLink(annot, document, arena);
    if (uri != null) {
      return PdfLink([rect], url: uri, annotation: annotation);
    }

    if (_isAnnotPushButton(annot, formHandle)) {
      return PdfLink([rect], isPushButton: true, annotation: annotation);
    }
